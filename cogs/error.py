...
class Error(commands.Cog):
    ...
    async def on_command_error(self, ctx: commands.Context, error):
        ...
        base = AuthorEmbed(ctx.author, title="이런!", display_footer=True, color=EmbedColor.NEGATIVE)
        ...
        if isinstance(error, commands.CommandNotFound):
            ...
            if conf:
                debug_format = f"===JBOT-DEBUG===\n" \
                               ...
                               f"{tb}\n" \
                               f"===END-DEBUG==="
                with open(f"traceback/{str(time.time()).split('.')[0]}.txt", "w", encoding="UTF-8") as f:
                    f.write(debug_format)
                await self.bot.get_channel(696562545489870861).send("새로운 오류가 전송되었습니다.")
